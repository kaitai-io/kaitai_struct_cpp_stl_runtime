...
void gif_t::_read() {
    ...
    m_logical_screen_descriptor->_read();
    ...
    m_blocks = new std::vector<block_t*>();
    {
        ...
        block_t* _;
        do {
            block_t* _t_blocks = new block_t(m__io, this, m__root);
            _t_blocks->_read();
            _ = _t_blocks;
...
void gif_t::_write() {
    m_hdr->_write();
    m_logical_screen_descriptor->_write();
    if (logical_screen_descriptor()->has_color_table()) {
        std::stringstream raw_global_color_table_stream;  // TODO needs to avoid reconstructing object
        m_global_color_table->_io()->set_stream(&raw_global_color_table_stream);
        m_global_color_table->_write();
        std::string raw_global_color_table = raw_global_color_table_stream.str();  // TODO needs to avoid reconstructing object
        m__io->write_bytes(raw_global_color_table, (logical_screen_descriptor()->color_table_size() * 3));
        m_global_color_table->_io()->set_stream(m__raw_global_color_table);
    }
    // TODO
}
...
void gif_t::image_data_t::_write() {
}
...
void gif_t::color_table_entry_t::_write() {
    m__io->write_u1(m_red);
    m__io->write_u1(m_green);
    m__io->write_u1(m_blue);
}
...
void gif_t::logical_screen_descriptor_struct_t::_write() {
    m__io->write_u2le(m_screen_width);
    m__io->write_u2le(m_screen_height);
    m__io->write_u1(m_flags);
    m__io->write_u1(m_bg_color_index);
    m__io->write_u1(m_pixel_aspect_ratio);
}
...
void gif_t::local_image_descriptor_t::_read() {
    ...
    if (has_color_table()) {
        ...
        m_local_color_table->_read();
    }
    m_image_data = new image_data_t(m__io, this, m__root);
    m_image_data->_read();
}

void gif_t::local_image_descriptor_t::_write() {
}
...
void gif_t::color_table_t::_read() {
    m_entries = new std::vector<color_table_entry_t*>();
    {
        int i = 0;
        while (!m__io->is_eof()) {
            color_table_entry_t* _t_entries = new color_table_entry_t(m__io, this, m__root);
            _t_entries->_read();
            m_entries->push_back(_t_entries);
            i++;
        }
    }
}

void gif_t::color_table_t::_write() {
    int i = 0;
    while (i < m_entries->size()) {
        color_table_entry_t* _t_entries = (*m_entries)[i];
        _t_entries->_write();
        i++;
    }
}
...
void gif_t::header_t::_read() {
    m_magic = m__io->read_fixed_contents(std::string("\x47\x49\x46", 3));
    m_version = kaitai::kstream::bytes_to_str(m__io->read_bytes(3), std::string("ASCII"));
}

void gif_t::header_t::_write() {
    m__io->write_fixed_contents(m_magic, std::string("\x47\x49\x46", 3));
    m__io->write_bytes(kaitai::kstream::str_to_bytes(m_version, std::string("ASCII")), 3);
}
...
void gif_t::ext_graphic_control_t::_read() {
    m_block_size = m__io->read_fixed_contents(std::string("\x04", 1));
    m_flags = m__io->read_u1();
    m_delay_time = m__io->read_u2le();
    m_transparent_idx = m__io->read_u1();
    m_terminator = m__io->read_fixed_contents(std::string("\x00", 1));
}

void gif_t::ext_graphic_control_t::_write() {
}
...
void gif_t::ext_application_t::_read() {
    ...
    m_application_id->_read();
    m_subblocks = new std::vector<subblock_t*>();
    {
        int i = 0;
        subblock_t* _;
        do {
            subblock_t* _t_subblocks = new subblock_t(m__io, this, m__root);
            _t_subblocks->_read();
            _ = _t_subblocks;
...
void gif_t::ext_application_t::_write() {
}
...
void gif_t::subblocks_t::_read() {
    m_entries = new std::vector<subblock_t*>();
    {
        ...
        subblock_t* _;
        do {
            subblock_t* _t_entries = new subblock_t(m__io, this, m__root);
            _t_entries->_read();
            _ = _t_entries;
